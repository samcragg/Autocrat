...
public:
    ...
    explicit intrusive_ptr(T* pointer) noexcept : _pointer(pointer)
    {
        if (_pointer != nullptr)
        {
    ...
    intrusive_ptr(intrusive_ptr&& other) noexcept : _pointer(other._pointer)
    {
...
{
public:
    intrusive_ref_counter() noexcept : _counter(0)
    {
    }

    intrusive_ref_counter(const intrusive_ref_counter&) noexcept : _counter(0)
    {
    }
    ...
    friend void intrusive_ptr_add_ref(
        const intrusive_ref_counter<T>* pointer) noexcept
    {
        pointer->_counter++;
    }
