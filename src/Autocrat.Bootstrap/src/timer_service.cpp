...
timer_service::timer_service(thread_pool* pool) : _thread_pool(pool)
{
}

std::uint32_t timer_service::add_timer_callback(
    duration delay,
    duration interval,
...
void timer_service::check_and_dispatch()
{
    ...
    auto new_end = std::remove_if(
        _slots.begin(), _slots.end(), [=](const time_slot& slot) {
...
bool operator<(
    const timer_service::time_slot& a,
    const timer_service::time_slot& b)
{
    return a.due < b.due;
}
