...
namespace autocrat
{
    namespace detail
    {
        ...
        protected:
            ...
            std::optional<void*> get_moved_location(void* object) override
            {
                ...
                auto type = reinterpret_cast<std::uintptr_t>(static_cast<managed_object*>(object)->type);
                if ((type & moved_bit) == 0)
                {
                    return std::nullopt;
                }
            ...
            void set_moved_location(void* object, void* new_location) override
            {
                auto pointer = reinterpret_cast<std::uintptr_t>(new_location) | moved_bit;
            ...
            void set_reference(void* object, std::size_t offset, void* reference) override
            {
                ...
                _buffer->replace(index + offset, reinterpret_cast<std::byte*>(&value), sizeof(std::ptrdiff_t));
            }
        private:
    ...
    void* object_serializer::restore()
    {
        ...
        {
            spdlog::debug("Warning: Large object graph serialized, this could affect performance.");
            detail::deserializer<std::unordered_map<void*, void*>> d(buffer, _references);
            return d.restore();
        }
    }
