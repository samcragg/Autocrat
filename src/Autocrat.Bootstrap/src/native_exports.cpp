...
{
    auto* service =
        autocrat::global_services.get_service<autocrat::worker_service>();
...
extern "C"
{
    void CDECL load_object_guid(
        const void* type,
        managed_guid* id,
        typed_reference* result)
    {
        load_object(
            type,
            result,
            std::string_view(
                reinterpret_cast<char*>(&id->data), sizeof(id->data)));
    }

    void CDECL load_object_int64(
        const void* type,
        std::int64_t id,
        typed_reference* result)
    {
        ...
        typed_reference* result)
    {
        load_object(
            ...
            result,
            std::string_view(
                reinterpret_cast<char*>(id->data),
                id->length * sizeof(char16_t)));
    }
    ...
    std::int32_t CDECL register_timer(
        ...
        std::int32_t handle)
    {
        ...
        auto* service =
            autocrat::global_services.get_service<autocrat::timer_service>();
    ...
    void CDECL
    register_udp_data_received(std::int32_t port, std::int32_t handle)
    {
        auto callback =
            std::get<udp_data_received_method>(get_known_method(handle));
        auto* service =
            autocrat::global_services.get_service<autocrat::network_service>();
        service->add_udp_callback(static_cast<std::uint16_t>(port), callback);
    }

    void CDECL task_enqueue(managed_delegate* callback, void* state)
    {
        autocrat::global_services.get_service<autocrat::task_service>()
